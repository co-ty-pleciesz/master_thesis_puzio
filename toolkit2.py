...
def adjustSignalToIDTxl(eegSignal, containesEpochedData=0, epochIndices=None):
    ...
    eeg_data = eegSignal.get_data() 
    print(eeg_data.shape)  # Powinno zwrócić (n_channels, n_samples)
    ...
    if containesEpochedData:
        ...
        data = Data(
            data=eegSignal, dim_order="rps", normalise=True, seed=1
        )  # seed=1 to have replication of results

    else:
        data = Data(
            data=eeg_data, dim_order="ps", normalise=True, seed=1
        )  # seed=1 to have replication of results;
